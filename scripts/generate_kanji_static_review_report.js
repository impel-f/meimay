...
function main() {
  ...
  const rows = master.map((source, index) => {
    ...
    return { index: index + 1, kanji, inappropriate: Number(source['不適切フラグ']) === 1, ...details[kanji] };
  });
...
function render(){const q=document.querySelector('#q').value.trim().toLowerCase();const f=document.querySelector('#filter').value;const list=rows.filter(r=>{if(f==='usable'&&r.inappropriate)return false;if(f==='inappropriate'&&!r.inappropriate)return false;if(['source_grounded','cross_checked'].includes(f)&&r.etymology?.reviewStatus!==f)return false;const hay=[r.kanji,r.meaningSummary,r.meaningDetail,r.namingMeaning,r.etymology?.text,...(r.compounds||[]).flatMap(x=>[x.word,x.reading,x.meaning])].join(' ').toLowerCase();return !q||hay.includes(q)});document.querySelector('#summary').textContent=\`${'${list.length}'} / ${rows.length}字を表示\`;document.querySelector('#grid').innerHTML=list.map(r=>\`<article class="card ${'${r.inappropriate?\'inappropriate\':\'\'}'}"><div class="kanji"><span class="number">${'${r.index}'}</span>${'${esc(r.kanji)}'}<span class="status">${'${esc(r.nameUse?.category)}'}</span></div><div><div class="label">意味</div><div class="text">${'${esc(r.meaningDetail)}'}</div></div><div><div class="label">名づけでの意味</div><div class="text">${'${esc(r.namingMeaning)}'}</div></div><div class="wide"><div class="label">成り立ち</div><div class="text">${'${esc(r.etymology?.text)}'}</div><span class="status">${'${esc(r.etymology?.reviewStatus)}'}</span></div><div class="wide"><div class="label">代表的な熟語</div>${'${(r.compounds||[]).map(x=>\`<div class="text compound"><b>${esc(x.word)}</b>（${esc(x.reading)}）：${esc(x.meaning)}</div>\`).join(\'\')||\'<div class="text">掲載なし</div>\'}'}</div></article>\`).join('')};
