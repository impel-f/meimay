...
const DEEP_DIVE_REVIEW_PATH = path.join(ROOT, 'scripts', 'data', 'kanji_meaning_deep_dive_reviews.json');
...
function main() {
  ...
  const deepDiveReviews = JSON.parse(fs.readFileSync(DEEP_DIVE_REVIEW_PATH, 'utf8')).entries || {};
  const automatedAuditThrough = Number(codexReviewData.automatedAuditThrough) || 0;
  const rows = master.map((source, index) => {
    ...
    return {
      ...
      automatedAuditStatus: index < automatedAuditThrough ? 'audited' : 'pending',
      contentReviewedAt: codexReview.reviewedAt || '',
      deepDiveReviewStatus: deepDiveReviews[kanji]?.status === 'reviewed' ? 'reviewed' : 'pending',
      ...details[kanji],
    };
  });
...
function render(){const q=document.querySelector('#q').value.trim().toLowerCase();const f=document.querySelector('#filter').value;const list=rows.filter(r=>{if(f==='usable'&&r.inappropriate)return false;if(f==='inappropriate'&&!r.inappropriate)return false;if(f==='deep-pending'&&r.deepDiveReviewStatus!=='pending')return false;if(f==='deep-reviewed'&&r.deepDiveReviewStatus!=='reviewed')return false;if(f==='no_compounds'&&(r.compounds||[]).length)return false;if(['source_grounded','cross_checked'].includes(f)&&r.etymology?.reviewStatus!==f)return false;const hay=[r.kanji,r.meaningSummary,r.meaningDetail,r.meaningDeepDive,r.etymology?.text,r.compoundNotice,...(r.compounds||[]).flatMap(x=>[x.word,x.reading,x.meaning])].join(' ').toLowerCase();return !q||hay.includes(q)});const deepReviewed=rows.filter(r=>r.deepDiveReviewStatus==='reviewed').length;const audited=rows.filter(r=>r.automatedAuditStatus==='audited').length;document.querySelector('#summary').textContent=\`${'${list.length}'} / ${rows.length}字を表示・深掘り個別確認済み ${'${deepReviewed}'}字・個別確認待ち ${'${rows.length-deepReviewed}'}字・自動監査済み ${'${audited}'}字\`;document.querySelector('#grid').innerHTML=list.map(r=>\`<article class="card ${'${r.inappropriate?\'inappropriate\':\'\'}'}"><div class="kanji"><span class="number">${'${r.index}'}</span>${'${esc(r.kanji)}'}<span class="status">${'${esc(r.nameUse?.category)}'}</span><span class="status ${'${r.deepDiveReviewStatus===\'pending\'?\'pending\':\'\'}'}">${'${r.deepDiveReviewStatus===\'reviewed\'?\'深掘り個別確認済み\':\'深掘り個別確認待ち\'}'}</span></div><div><div class="label">辞書の詳細語義</div><div class="text">${'${esc(r.meaningDetail)}'}</div></div><div><div class="label">意味の深掘り</div><div class="text">${'${esc(r.meaningDeepDive)}'}</div></div><div class="wide"><div class="label">成り立ち</div><div class="text">${'${esc(r.etymology?.text)}'}</div><span class="status">${'${esc(r.etymology?.reviewStatus)}'}</span></div><div class="wide"><div class="label">代表的な熟語</div>${'${(r.compounds||[]).map(x=>\`<div class="text compound"><b>${esc(x.word)}</b>（${esc(x.reading)}）：${esc(x.meaning)}</div>\`).join(\'\')||\`<div class="text compound-notice">${esc(r.compoundNotice||\'検証済みの代表的な熟語は未掲載です。\')}</div>\`}'}</div></article>\`).join('')};
