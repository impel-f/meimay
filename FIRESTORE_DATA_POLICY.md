...
- 個人バックアップは `users/{uid}` に 1.2 秒デバウンスで同期し、60 秒ごとの保険同期もある。内容 fingerprint が同じなら再送しない。
- パートナー同期は `rooms/{roomId}/data/{uid}` に 1.2 秒デバウンスで同期し、相手は同じ doc を listener で購読する。
...
- Firestore 同期はデバウンスでまとめる。小さな連続操作を 1 回の `set(..., { merge: true })` に畳む。
...
- DAU が増えたら、個人バックアップは 3から5 秒デバウンス、パートナー同期は 1から2 秒デバウンス、最大待ち時間 15から30 秒の設計へ寄せる。
...
- 端末を失った場合は、エクスポートJSONのインポートを正式な自己復元手段にする。
- サーバー復元を入れるなら、`復元ID + 高エントロピー復元キー` を使う。復元キーはサーバー側でハッシュ化して保存し、平文は一度しか表示しない。
...
- 連携中に開始した場合、同じ room の両者が3日間使える代わりに、両者の trial を消費済みにする。
- 片方がすでに trial 消費済みの場合は、もう片方の無料枠だけで room 全体を開けない。片方だけ有効にすると体験が割れるため、UI では「このルームでは無料体験を開始できません」と説明する。
- 片方が有料プレミアム中なら trial は開始せず、有料権限を優先する。

案内を出す場所:
- プレミアムモーダルの購入ボタンより上。
- 1日のスワイプ上限に達したとき。
- 初めて保存済み名前を作った後。
- パートナー連携完了後、両者が未消費なら「二人で3日間試す」。
- 設定のプレミアム行。
