...
- 子ワークスペースの `updatedAt` / 子メタデータの `updatedAt` は、room sync と個人バックアップの fingerprint から外す。同期直前の保存時刻だけが変わっても Firestore へ再送しない。
...
- プロフィール変更は個別の即時 `set` ではなく、パートナー同期のデバウンスに集約する。プレミアム状態は同一 fingerprint なら再送しない。
