...
- Capacitor 設定、RevenueCat Capacitor SDK、`ios/App/App.xcodeproj`、Codemagic 用 `codemagic.yaml`、Xcode Cloud 用 `ci_post_clone.sh` は追加済み。Codemagic の `ios-capacitor-smoke` は成功済み。次は App Store Connect 連携 `codemagic` と Environment group `code-signing` の `CERTIFICATE_PRIVATE_KEY` を使う `ios-testflight` で App Store 配布用の署名ファイルを自動作成し、TestFlight / sandbox アカウントの購入画面を確認する。
