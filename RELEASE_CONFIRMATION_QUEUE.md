...
- Capacitor 設定、RevenueCat Capacitor SDK、`ios/App/App.xcodeproj`、Xcode Cloud 用 `ci_post_clone.sh` は追加済み。次は App Store Connect の Xcode Cloud で `main` / `ios/App/App.xcodeproj` / scheme `App` の Archive ワークフローを作り、TestFlight / sandbox アカウントで購入画面を確認する。
